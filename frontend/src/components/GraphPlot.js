...
class GraphPlot extends Component {
  ...
  render () {
    ...
    let xTickPositions = [...Array(data.length-2).keys()];
    xTickPositions.push(data.length-1);
    xTickPositions.push(data.length);
    ...
    return (
      ...
      >
        <XAxis
          ...
          tickFormat={v => (v === data.length || v === data.length-1) ? dateTimes[v-1] : dateTimes[v]} 
        />
        <YAxis tickFormat={v => '$'+v.toFixed(2)} />
        <VerticalGridLines style={{stroke: '#243039'}}  tickValues={xTickPositions} />
